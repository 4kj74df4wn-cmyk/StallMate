...
function initAuthLayer(){
  ...
  // ---- binding-state tracking (PERMANENT latch: once ever bound, legacy fallback OFF forever) ----
  var __isBound = null;            // null=indeterminate, false=confirmed NO binding, true=bound
  var __fallbackLatchOff = false;  // once true, legacy fallback permanently disabled (never re-opens)
  var __fallbackCount = 0;
  function _applyBound(exists){ if (exists){ __isBound = true; __fallbackLatchOff = true; } else if (__isBound !== true){ __isBound = false; } }
  try{ fbDb.ref('roomOwners/'+ROOM).on('value', function(s){ _applyBound(!!(s && s.exists())); }); }catch(e){}
  // resolve binding state before deciding; error => stays indeterminate (null) => fail-safe (queue, never legacy)
  function ensureBindingKnown(){
    if (__isBound !== null) return Promise.resolve(__isBound);
    return fbDb.ref('roomOwners/'+ROOM).once('value').then(function(s){ _applyBound(!!(s && s.exists())); return __isBound; }, function(){ return null; });
  }
  // every legacy fallback emits countable telemetry
  function emitFallbackTelemetry(){ __fallbackCount++; try{ fbDb.ref('readiness/audit/'+deviceId).update({ fallbackCount: __fallbackCount, lastFallbackAt: firebase.database.ServerValue.TIMESTAMP, appVersion: AUTH_BUILD }); }catch(e){} }
  function __markSynced(key, ok, rec){ try{ if (typeof unsyncedIds!=='undefined'){ if(ok) unsyncedIds.delete(key); else unsyncedIds.add(key); if(typeof saveUnsynced==='function')saveUnsynced(); if(typeof refreshSyncInfoText==='function')refreshSyncInfoText(); } if(rec) banner('⚠️ v'+AUTH_BUILD+' ต้องกู้รายการขาย (opid_conflict) — ตรวจสอบ', true); }catch(e){} }

  // Hardened sale entry — ALL sale paths route here. Legacy fallback ONLY when positively confirmed
  // PRE_BINDING (bound===false) AND never latched. Auth/network/callable errors or indeterminate state
  // NEVER open legacy: they queue via guardedSaleWrite (immutable snapshot, amount unchanged, dedup by
  // opId, no loss/dup). Once ever bound => latch off => legacy permanently disabled (also can't bypass
  // Rules B since Rules B implies bound). Every legacy fallback emits telemetry.
  var __origUpdateSale = (typeof updateSaleInFirebase==='function') ? updateSaleInFirebase : null;
  function guardedSaleEntry(sale){
    var key = String(sale.id).replace('.','_');
    var snap = Object.assign({}, sale, { __opId: key });
    var doGuarded = function(){ return ctrl.guardedSaleWrite(snap, detWriter).then(function(r){ __markSynced(key, r&&r.ok, r&&r.recoveryRequired); return r; }); };
    return ensureBindingKnown().then(function(bound){
      return ctrl.isOwnerAuthorized().then(function(authed){
        if (authed) return doGuarded();
        if (!__fallbackLatchOff && bound === false && __origUpdateSale){
          emitFallbackTelemetry(); __origUpdateSale(sale); return { ok:true, fallback:true };   // PRE_BINDING only
        }
        return doGuarded();   // bound-but-not-authed OR indeterminate/error => queue, NEVER silent legacy
      });
    });
  }
  window.updateSaleInFirebase = function(sale){ try{ return guardedSaleEntry(sale); }catch(e){ try{ return guardedSaleEntry(sale); }catch(e2){} } };

  // route the legacy whole-array sales write (import/restore path pushToFirebase('all'|'sales'))
  // through the guarded per-record writer — no sale ever bypasses the guarded writer.
  var __origPush = (typeof pushToFirebase==='function') ? pushToFirebase : null;
  if (__origPush){
    window.pushToFirebase = function(changed){
      if (changed==='sales' || changed==='all'){
        try{ (typeof sales!=='undefined' && sales ? sales : []).forEach(function(s){ window.updateSaleInFirebase(s); }); }catch(e){}
        if (changed==='all'){ try{ __origPush('menu'); }catch(e){} try{ __origPush('cfg'); }catch(e){} }
        return; // never write roomPayload.sales directly
      }
      return __origPush(changed);
    };
  }
