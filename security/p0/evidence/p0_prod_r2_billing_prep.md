...
- **deps ตอน gate (reproducible, HOLD-5 fix):** smoke ใช้ firebase **client** SDK. มี **dedicated runner manifest** ที่ `security/p0/evidence/` (มีแค่ `firebase` + `firebase-admin`) **แยกจาก** `security/p0/package.json` เพื่อเลี่ยง peer conflict ของ `@firebase/rules-unit-testing` (บังคับ firebase ^12 ชนกับ ^10.14.1 → `npm ci` ERESOLVE). รันจาก `security/p0/evidence`:
  ```bash
  cd security/p0/evidence
  npm ci                                  # จาก committed lockfile (ห้าม npm i / --force / --legacy-peer-deps)
  node p0_prod_r2_postdeploy_smoke.js
  ```
  ยืนยันแล้ว: **`npm ci` exit 0** (206 packages, ไม่มี ERESOLVE) + `node --check` ผ่าน + firebase client/admin resolve ได้ (ดู guard results §D)
- **smoke-artifact guard (HOLD-5):** `./prod/verify_smoke_artifacts_sha.sh` ตรวจ 3 ไฟล์ dedicated fail-closed: `evidence/package.json` (`94520d6b…`), `evidence/package-lock.json` (`c397d2d8…`), `evidence/p0_prod_r2_postdeploy_smoke.js` (`e7211eac…`)
