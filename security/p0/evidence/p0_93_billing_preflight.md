> ⚠️ **SUPERSEDED (2026-09-10).** เอกสารฉบับ 3 ก.ย. นี้ **ล้าสมัย** — ระบุ `maxInstances 3–5` (ค่าเก่า) และพูดถึงทั้ง `bindOwner` + `redeemLicense`. ค่าที่อนุมัติจริงคือ **maxInstances=1 / concurrency=1 / timeout=30 / asia-southeast1** และรอบนี้เตรียม **เฉพาะ `bindOwner`**. ใช้ `p0_prod_r2_billing_prep.md` + `p0_prod_r2_sha_manifest.json` แทน. เก็บไว้เป็นประวัติเท่านั้น.

# SECURITY P0 §9.3 — BILLING PREFLIGHT (DOCUMENT ONLY — Blaze NOT activated) [SUPERSEDED]

**Authorization:** Room 00 — §9.3 PREFLIGHT ONLY. **Do NOT activate or upgrade Blaze.** No deploy, no live binding.
**Date:** 3 Sep 2026. **[SUPERSEDED by p0_prod_r2_billing_prep.md — 10 ก.ย. 2026]**
